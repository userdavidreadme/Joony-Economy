...
module.exports = {
   ...
   */
    async execute(message, args) {
        let GetBypass = GetUserID(message.author.id)
        let Plant = await GetPlant(message.member.id)
        let getenchant = await GetEnchant(message.author.id, 'Fortune')
        ...
        if (verification > 29) {
            ...
            if (Plant) {
                if (getenchant === 'Fortune') { 
                    ...
                    .setDescription(`A BLESSING FROM THE SUN GOD! ☘💎🌴 FORTUNE ENCHANTMENT! You harvested <:Coin:935379171897643120> ${withoutbonus} and ${fortune} as a FORTUNE!\n👛 You now have <:Coin:935379171897643120> ${coins}, You are already rich haha.\nDoes anyone want this enchantment as well? do \`.lab\` to find out more!`)
                    .setFooter({text: `You are using ${plants[Plant]['name']} to harvest right now | ENCHANTMENT 🤑☘👩‍🌾 FORTUNE`})
                    .setTimestamp()
                    .setColor('BLUE')
